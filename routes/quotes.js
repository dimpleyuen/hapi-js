exports.register = function(server, options, next) {
  ...
  var db = [
   ...
   {quote: "'Xavi just blew my mind' - Elia 'Wait.. when did I blow someone?'", author: "XLS"},
   ...
   {quote: "Do you want me to hold it for you so you can do it with two hands?", author: "Dimple"},
  ...
  //here we will write our routes for the API
  server.route([
    { //hello world
      method: 'GET',
      path: '/',
      handler: function(request, reply) {
        reply("Hello World! I'm A HarryQuote API Server!");
      }
    },
    { //get all quotes
      method: 'GET',
    ...
    { //get quote by id
     ...
     handler: function (request, reply) {
       ...
       if (isNaN(id) || id > db.length) {
         return reply ("Gimme a Number Bro!").code(404);
       }
       reply( db[request.params.id] );
       }
    },
    { //get quotes by name, case sensitive and must be exact match
      method: 'GET',
      path: '/quotes/author/{author}',
      handler: function (request, reply) {
        var wantedAuthor = request.params.author;
        authorDB = [];
        
        for (var i = 0 ; i < db.length ; i++) {
          if (db[i].author === wantedAuthor) {
            authorDB.push(db[i]);
          }
        }
        reply (authorDB);
